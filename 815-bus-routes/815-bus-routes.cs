public class Solution {
    public int NumBusesToDestination(int[][] routes, int src, int destination) {
       if (src == destination)
       {
            return 0;
       }
       ...
       for (int i = 0; i < routes.Length; i++)
       {
        ...
        if (!stopToBuses.ContainsKey(src) ||
           !stopToBuses.ContainsKey(destination))
        {
            return -1;
        }
        
        return BFS(src, destination, busToStops, stopToBuses);
    }
